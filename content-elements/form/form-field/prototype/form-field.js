...
Alpine.data("formField", () => ({
  ...
  validateInput() {
    ...
    if (this.inputEl.checkValidity()) {
      this._setTooltipAccessibility(this.fp ? this.fp.altInput.id : this.inputEl.id);
  ...
  _initDateInput() {
    ...
    let _dateTimeCast = (dateStr) =>
      dateStr
        ? new Date(this.isTime ? `2000-01-01T${dateStr}` : dateStr)
        : null;
    ...
    this.fp.altInput.id = this.inputEl.id;
    this.inputEl.removeAttribute("id");
    this._setTooltipAccessibility(this.fp.altInput.id);
